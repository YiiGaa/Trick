...
| _value | 内容对应的key（i18next配置的key） |  | String | 1.0=2024.11.15 |
| _config | 翻译设置，变量替换             |                       | Object | 1.0=2024.11.15 |
| _html | 翻译设置，html标签替换 | | Object | 1.0=2024.11.15 |

参数补充说明：

### > _value _config

- _value, _config的设置，参考`_text`说明，为普通翻译配置

### > _html

- _html为特殊的翻译设置，可以替换翻译文本中的html标签

- 例如，翻译文本中标记了`<a>xxx</a>`，可以设置以下值，将自动修改`a`标签的属性
...
- 推荐格式为`comp##_FunTranslate<<[所在页面布局]`，如`comp##_FunTranslate<<Title`、`comp##_FunTranslate<<Title/Info`
...
也可以通过`Christmas插件`运行`ShellExcute>>Build#Component`，并在插件打开的终端中输入`_FunTranslate`
