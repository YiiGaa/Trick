# _BoxCollapse-Collapse and hide content

The main functions include:

- Config Settings

- switch-switch display/hide
...
| _initOpen | When initializing, whether to display content, true: display, false: hide | true | Bool | 1.0=2024.11.15 |
| _templButton | Display/Hide button template | Take the whole `child` as the template | Open type | 1.0=2024.11.15 |
| _templPanel | Content template | Take the whole `child` as the template | Open type | 1.0=2024.11.15 |
| _classBody | Outer style of the component | | String | 1.0=2024.11.15 |
| _classButton | Outer style settings of the Open/Hide Button | | String | 1.0=2024.11.15 |
| _classPanel | Outer style settings of the content | | String | 1.0=2024.11.15 |
...
- The recommended format is `comp##_BoxCollapse<<[page layout]`, such as `comp##_BoxCollapse<<Title`, `comp##_BoxCollapse<<Title/Info`
...
| key | Description | Default | Type | Add/Update |
| ------- | ------------------------------------ | ------------ | ----- | ----- |
| _action | Fixed as "switch" | | String | 1.0=2024.11.15 |
| _isOpen | Whether to display content, true: display content, false: hide content | Automatic switching status | Bool | 1.0=2024.11.15 |
...
- If this value is omitted, the status will be automatically switched
...
Data will be automatically inserted into `passParam`, with the insertion field being the value corresponding to `_resultKey`. When the value corresponding to `_resultKey` is an empty string `""`, the entire `passParam` will be cleared and the data will be written
...
Python3 Christmas/Christmas.py ShellExcute/Build#Component _BoxCollapse

```

You can also run `ShellExcute>>Build#Component` through `Christmas plug-in` and enter `_BoxCollapse` in the terminal where the plug-in is opened
