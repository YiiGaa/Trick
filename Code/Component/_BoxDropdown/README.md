...
- 对象`{}`参数设置如下
    ...
    | _classItem  | 菜单项样式                                 |                       | String   | 1.0=2024.11.15 |
    | _onClick    | 菜单项点击回调，返回当前菜单项的数据       | 不回调                | 开放类型 | 1.0=2024.11.15 |
    | _data       | 自定义暂存数据，菜单项点击回调时会一并返回 | 不回调                | 开放类型 | 1.0=2024.11.15 |
