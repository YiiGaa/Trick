...
**This module cannot work normally due to the bug of the basic library headlessui
...
| _isShow | Whether to display animation sub-items, true: display, false: hide | true                       | Bool      | 1.0=2024.11.15 |
| _map    | Animation sub-item settings                                  |                            | Array     | 1.0=2024.11.15 |

Parameter Supplementary Description:

### > _map

- Animation sub-item template settings need to be array embedded object `[{},{}]`

- The parameter settings of the object `{}` are as follows

    | key         | Description                                                  | Default value                          | Type   | Group/Update version |
    | ----------- | ------------------------------------------------------------ | -------------------------------------- | ------ | -------------------- |
    | _templ      | Animation sub-item template, please refer to `_templ` instructions | Take the whole `child` as the template | String | 1.0=2024.11.15       |
...
- The recommended format is `comp##_BoxMotion<<[page layout]`, such as `comp##_BoxMotion<<Title`, `comp##_BoxMotion<<Title/Info`
...
| _isShow      | Whether to display animation sub-items, true: display/replay animation, false: hide | Invert the current value | Bool   | 1.0=2024.11.15 |
| _refreshTime | Animation replay interval, unit: milliseconds                | 500                      | Int    | 1.0=2024.11.15 |

Supplementary description of parameters:

### > _isShow

- Whether to display animation sub-items, true: display/replay animation, false: hide
- The current `_isShow` value is true, and the `_isShow` setting value is also true, indicating that the animation should be replayed

### > _refreshTime

- Animation replay interval, the current `_isShow` value is true, and the `_isShow` setting value is also true to take effect
- The setting of `_refreshTime` needs to depend on the duration of the element's disappearance animation. If there is no special requirement, just keep the default
...
```
Python3 Christmas/Christmas.py ShellExcute/Build#Component _BoxMotion

```

You can also run `ShellExcute>>Build#Component` through `Christmas plug-in` and enter `_BoxMotion` in the terminal where the plug-in is opened
