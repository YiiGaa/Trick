...
| _isShow | 是否显示动画子项，true：显示，false：隐藏 | true | Bool | 1.0=2024.11.15 |
| _map | 动画子项设置                    |                       | Array/Object | 1.0=2024.11.15 |

参数补充说明：

### > _map

- 动画子项设置，可以是数组内嵌对象`[{},{}]`，或者单个对象`{}`

- 对象`{}`参数设置如下

    |         key | 说明                             | 默认值                | 类型   | 组入/更新版本  |
    | ----------: | -------------------------------- | --------------------- | ------ | -------------- |
    |      _templ | 动画子项模板，请参考`_templ`说明 | 以整个`child`作为模板 | String | 1.0=2024.11.15 |
...
- 推荐格式为`comp##_BoxMotion<<[所在页面布局]`，如`comp##_BoxMotion<<Title`、`comp##_BoxMotion<<Title/Info`
...
| _isShow      | 是否显示动画子项，true：显示/重新播放动画，false：隐藏 | 当前值取反 | Bool   | 1.0=2024.11.15 |
| _refreshTime | 动画重新播放间隔，单位：毫秒                           | 500        | Int    | 1.0=2024.11.15 |

参数补充说明：

### > _isShow

- 是否显示动画子项，true：显示/重新播放动画，false：隐藏
- 当前`_isShow`值为true，且`_isShow`设置值也为true，表示重新播放动画

### > _refreshTime

- 动画重新播放间隔，当前`_isShow`值为true，且`_isShow`设置值也为true时才生效
- `_refreshTime`的设置，需要根据元素消失动画的时长而定，若无特别要求，保持默认即可
...
也可以通过`Christmas插件`运行`ShellExcute>>Build#Component`，并在插件打开的终端中输入`_BoxMotion`
